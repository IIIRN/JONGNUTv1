...
import { saveNotificationSettings, saveBookingSettings, savePointSettings, savePaymentSettings } from '@/app/actions/settingsActions';
...
export default function AdminSettingsPage() {
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            setLoading(true);
            try {
                const docsToFetch = ['notifications', 'booking', 'points', 'payment'];
                const promises = docsToFetch.map(id => getDoc(doc(db, 'settings', id)));
                const [notificationsSnap, bookingSnap, pointsSnap, paymentSnap] = await Promise.all(promises);
    ...
    const handleNotificationChange = (group, key, value) => {
        ...
        setIsSaving(true);
        try {
            let paymentData = { ...paymentSettings };
            if (qrCodeFile) {
                const storage = getStorage();
                const storageRef = ref(storage, `payment_qrcodes/${qrCodeFile.name}_${Date.now()}`);
                await uploadBytes(storageRef, qrCodeFile);
                paymentData.qrCodeImageUrl = await getDownloadURL(storageRef);
                setPaymentSettings(prev => ({...prev, qrCodeImageUrl: paymentData.qrCodeImageUrl}));
            }

            // --- ใช้ Destructuring เพื่อกรอง field 'updatedAt' ออก ---
    ...
    return (
        <div className="container mx-auto p-4 md:p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                ...
                {/* --- Column 1: Booking, Holidays, Payment --- */}
                    ...
                    <SettingsCard title="วันหยุดพิเศษ">
                        <div className="flex gap-2 items-end">
                            <div className="flex-1">
                                ...
                                <input 
                                    type="date" value={bookingSettings._newHolidayDate || ''} 
                        ...
                        <div className="space-y-1 max-h-24 overflow-y-auto border rounded-lg p-2 mt-2">
                     ...
                     <SettingsCard title="การตั้งค่าการชำระเงิน">
                        ...
                        {paymentSettings.method === 'image' && (
                            <div>
                                <label className="block text-xs font-medium text-gray-700 mb-1">อัปโหลด QR Code</label>
                                <input type="file" accept="image/*" onChange={e => setQrCodeFile(e.target.files[0])} className="text-xs"/>
                                {paymentSettings.qrCodeImageUrl && <img src={paymentSettings.qrCodeImageUrl} alt="QR Code" className="w-24 h-24 mt-2 border"/>}
                            </div>
                        )}
                    </SettingsCard>
                </div>
